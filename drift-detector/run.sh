...
# Drift Detector Run Script - Automatically sets up Claude and ConfigHub
...
echo -e "${BLUE}🔍 Drift Detector with Claude AI${NC}"
echo "===================================="
echo ""
...
if [ "$ENABLE_CLAUDE" = "false" ]; then
    ...
    if [ -z "$CLAUDE_API_KEY" ] || [ "$CLAUDE_API_KEY" = "your-claude-api-key-here" ]; then
        ...
        if [ -n "$input_key" ]; then
            ...
            echo -e "${YELLOW}⚠️  Continuing without Claude (basic drift detection only)${NC}"
        fi
    else
...
# Build if needed
if [ ! -f drift-detector ] || [ main.go -nt drift-detector ]; then
    echo -e "${YELLOW}🔨 Building drift-detector...${NC}"
    go build -o drift-detector .
    echo -e "${GREEN}✅ Build complete${NC}"
fi

echo ""
echo "Starting drift-detector with:"
echo "  • ConfigHub: ✅ Enabled"
if [ -n "$CLAUDE_API_KEY" ]; then
    echo "  • Claude AI: ✅ Enabled (root cause analysis)"
else
    echo "  • Claude AI: ❌ Disabled (basic drift detection only)"
fi
echo ""
